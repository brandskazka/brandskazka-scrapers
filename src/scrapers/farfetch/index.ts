...
// Load brands from selected_brand.json
const selectedBrands: string[] = JSON.parse(
  readFileSync('./src/scrapers/farfetch/selected_brand.json', 'utf8')
);

// Define the genders and assign the same selected brands to each
const genders: { id: string; name: Gender; brands: string[] }[] = [
  { id: "141259", name: "Men", brands: selectedBrands },
  { id: "141258", name: "Women", brands: selectedBrands },
  { id: "141260", name: "Kids", brands: selectedBrands }
];

// Find the active gender based on a given gender variable
...
export const getAllProducts = async (
  ...
  const products_in_rub: GeneralSearchResultProductItem[] = []; // Array to store all scraped products
  const products_in_euro: GeneralSearchResultProductItem[] = [];
  ...
  for (const product of json.products.entries) {
    ...
    products_in_euro.push({
      ...
      variants: single_product_json.variants.map((variant: any) => ({
        ...
        price: variant.price.priceInclTaxes * 1.05,
      })),
      slug: generateProductUrlHandle({
        id: single_product_json.id,
        shortDescription: single_product_json.shortDescription,
      ...
      price:
        single_product_json.variants?.[0]?.price?.priceInclTaxes * 1.05,
    });
  }
  ...
  return {
    ...pagination,
    products_in_rub: products_in_rub,
    products_in_euro: products_in_euro
  };
};
